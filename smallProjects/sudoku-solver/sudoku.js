...
class Sudoku {
  constructor(boardString) {
    this.string = boardString; // string for the sudoku box
  ...
  checkVertical(board, value, column) { // column = checked column (column of the blank box)
    for (let i = 0; i < 9; i++) {
  ...
  checkHorizontal(board, value, row) { // row = checked row (row of the blank box)
    for (let i = 0; i < 9; i++) {
  ...
  allChecks (board, value, row, column) {
    if (this.check3x3(board, value, row, column) &&
      this.checkHorizontal(board, value, row) &&
      this.checkVertical(board, value, column)
     ) {
      return true;
    }
    return false;
  ...
  solve() {
    ...
    while (i < blanks.length) {
      ...
      while (isSolved === false && value <= 9) {
        if (this.allChecks(board, value, blankRow, blankColumn)) {
          isSolved = true;
