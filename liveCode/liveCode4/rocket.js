...
function nextTargetArea(code) {
  ...
  for (var i = 0; i < code.length; i++) {
    var listKey = Object.keys(letterList);
    if (listKey.indexOf(code[i]) === -1) {
      letterList[code[i]] = 1;
    } else {
      letterList[code[i]] += 1;
    }
  }
  if (letterList.W !== undefined) {
    district += (letterList.W * 2);
  }
  if (letterList.F !== undefined) {
    district += (letterList.F * 4);
    letterList.R -= 1;
  }
  if (letterList.R !== undefined) {
    district += (letterList.R * 3);
  }
  if (letterList.X !== undefined) {
    district += (letterList.X * 6);
  }
  if (letterList.N !== undefined) {
    district += (letterList.N * 7);
  }
  if (letterList.G !== undefined) {
    district += (letterList.G * 8);
  }
